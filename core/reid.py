...
class ReID:
    def __init__(self,
                 ...
                 jacket_dist_thresh=50,       # px — khoảng cách tối đa cho spatial gating
                 jacket_time_thresh=90,        # frames = 3s @ 30fps
                 long_term_thresh=18000,       # frames = 10 phút @ 30fps (staircase/out)
                 makeup_thresh=3900,           # frames = 5 phút @ 13fps (makeup zone)
                 stage2_dist_thresh=200,       # px — ràng buộc vị trí cho Stage 2 hidden
                 gallery_update_interval=5,    # realtime: extract OSNet mỗi N frames
                 fps=13.0,
                 device='cpu'):
        ...
        self.makeup_thresh = makeup_thresh
        self.stage2_dist_thresh = stage2_dist_thresh
        self.gallery_update_interval = gallery_update_interval
        self.fps = fps
        self.max_gallery_size = 20
        ...
        self.gallery = {}       # {actual_id: {features, last_pos, last_frame, entry_frame, status, out_zone}}
        self.id_map = {}        # {bytetrack_id: actual_id}
        self.frame_counts = {}  # {actual_id: số frame đã thấy} — dùng cho gallery_update_interval
        self.next_id = 1
    ...
    def _match_with_gallery(self, current_feat, current_pos, current_frame):
        ...
        for track_id, data in self.gallery.items():
            ...
            effective_thresh = self.makeup_thresh if out_zone == 'makeup' else self.long_term_thresh
            ...
            if time_gap < self.jacket_time_thresh and dist < self.jacket_dist_thresh:
                return track_id, 1.0
    ...
    def _update_gallery(self, actual_id, feature, foot_pos, frame_idx):
        if actual_id not in self.gallery:
            self.gallery[actual_id] = {
                'features': deque(maxlen=self.max_gallery_size),
                'last_pos': foot_pos,
        ...
        self.gallery[actual_id]['status'] = 'active'
        self.gallery[actual_id]['out_zone'] = None
    ...
    def load_stub(self, stub_path):
        ...
        for data in payload['gallery'].values():
            data['features'] = deque(data['features'], maxlen=self.max_gallery_size)
    ...
    def process_frame(self, frame, frame_data, frame_idx):
        ...
        Tối ưu CPU: chỉ extract OSNet cho ID mới hoặc mỗi gallery_update_interval frame.
        """
        ...
        for old_id, track_info in frame_data.items():
            ...
            if is_new:
                ...
                # ID đã biết → chỉ extract feature định kỳ để cập nhật gallery
                count = self.frame_counts.get(actual_id, 0)
                feature = self.extract_feature(frame, bbox) if count % self.gallery_update_interval == 0 else None

            self._update_gallery(actual_id, feature, foot_pos, frame_idx)
            self.frame_counts[actual_id] = self.frame_counts.get(actual_id, 0) + 1
